<script>
  ...
  $: if (periodicTasks) {
    ...
    sortedWeeklyTasks = sortedPeriodicTasks
      .filter((task) => task.repeatOnDayOfWeek)
      .sort((a, b) => a.orderValue - b.orderValue);
    sortedMonthlyTasks = sortedPeriodicTasks
      .filter((task) => task.repeatOnDayOfMonth)
      .sort((a, b) => a.orderValue - b.orderValue);
  }
  ...
  function updateOrderOfDraggedTemplate(newVal) {
    updateFirestoreDoc(
      `users/${$user.uid}/periodicTasks/${draggedTemplate.id}`,
      {
        orderValue: newVal,
      }
    );
        ...
        {#each sortedWeeklyTasks as weeklyTask, i (weeklyTask.id)}
          {#if i === 0}
            <ReusableSimpleDropzone
              ...
              aboveOrder={0}
              belowOrder={sortedWeeklyTasks[0].orderValue}
            />
            <!-- general case drop-zone: must be between 2 tasks-->
          {:else if i > 0 && i < periodicTasks.length}
            <ReusableSimpleDropzone
              on:new-order-value={(e) => updateOrderOfDraggedTemplate(e.detail)}
              aboveOrder={sortedWeeklyTasks[i - 1].orderValue}
              belowOrder={sortedWeeklyTasks[i].orderValue}
            />
          {/if}
            ...
            >
              ...
              {#if getStatsFromTaskID[weeklyTask.id]}
                <div style="margin-left: 8px;">
                  <div
                    style="display: flex; align-items: center; margin-top: 8px; margin-bottom: 0px;"
                  >
                    {#each { length: getStatsFromTaskID[weeklyTask.id].frequency } as _, i}
                      <div
                        style="background: green; border-radius: 4px; 
                        width: {getDisplayLength(
                          weeklyTask
                        )}px; height: 3px; margin-right: 2px;"
                      ></div>
                    {/each}
                  </div>
                  <div
          ...
          {#if i === sortedWeeklyTasks.length - 1}
            <ReusableSimpleDropzone
              on:new-order-value={(e) => updateOrderOfDraggedTemplate(e.detail)}
              aboveOrder={sortedWeeklyTasks[i].orderValue}
              belowOrder={sortedWeeklyTasks[i].orderValue + 1}
            />
          {/if}
        {/each}
      </div>
...
<style src="./ManageReusableTasks.css" lang="postcss"></style>
