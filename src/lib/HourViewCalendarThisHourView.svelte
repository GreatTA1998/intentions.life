<div id="scroll-container" style="position: relative; width: 15vw">
  <div id="calendar-month-container" 
    style="height: {pixelsPerMinute * numOfTimestampsDisplayed}px; 
  ...
  >
    ...
    {#if calendarTimestamps}
      {#each calendarTimestamps as dayOfWeek, i}
        <div class="timestamp-number" style="top: {-6 + 6 + (pixelsPerMinute * i)}px;">
          {dayOfWeek}
        </div>
      {/each}
    {/if}
    ...
    {#each {length: 60 * numOfTimestampsDisplayed} as _, i}
      <div 
        class:visible-line={(i % 60) === 0}
        style="height: {pixelsPerMinute / 60}px; box-sizing: border-box; margin-right: 0; margin-left: auto; width: 82%"
...
<script>
  ...
  // let calendarTimestamps = ['0', '5', '10', '15', '20']

  let calendarTimestamps = [] 

  function generateCalendarTimestamps () {
    for (let i = 0; i <= 60; i += 5)
    calendarTimestamps.push(`${i}`)
  }

  generateCalendarTimestamps()

  const dispatch = createEventDispatcher()
  const numOfTimestampsDisplayed = 1 + (60 / 5)
